...
Function/Wave AbsorptionDifferentiateWave(wavInput, numSmooth, type)
	...
	if (Dimsize(wavInput, 1) == 3)
		...
		Duplicate/O/R=[][0]/FREE wavSmooth output
		Redimension/N=(-1,0) intensity, wavelength, output
		Differentiate/METH=(type) intensity/D=output/X=wavelength
		wavOutput[][0] = output[p]
	else
...
Function/Wave AbsorptionSmoothWave(wavInput, numSmooth)
	...
	if (numSmooth>0)
		if (Dimsize(wavInput, 1) == 3)
			Duplicate/O/R=[][0]/FREE wavInput intensity
...
Function/Wave AbsorptionPeakFind(wavInput, [sorted, redimensioned, differentiate2])
	...
	if (numColumns == 0)
        ...
        if(differentiate2)
            wave wavYdata = AbsorptionDifferentiate2Wave(wavInput, 10)
            wavYdata *= -1
        else
